/*
Dato un file csv contenente delle canzoni, riprodurle in maniera causale
*/
...
int main (){
   ...
   //Chiamo la funzione per memorizzare i dati della playlist nel vettore di canzoni
    ...
    while(fgets(riga, 1000, fp)){
        ...
        field = strtok(riga, ",");
        playlist[counter].numero_canzone = atoi(field);
        field = strtok(NULL, ",");
        playlist[counter].nome_canzone = strdup(field);
        field = strtok(NULL, ",");
        playlist[counter].autore = strdup(field);
        counter++;
        printf("%d\n", playlist[counter].numero_canzone);
        printf("%s\n", playlist[counter].nome_canzone);
        printf("%s\n", playlist[counter].autore);
    }
